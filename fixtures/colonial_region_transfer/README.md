...
- `event xcrt_acceptance.20`：9 种可通用构造的领土型直属附属国与
  `state_bank`/`trade_company` 两种 building 型直属附属对象的基线候选矩阵；`.21`
  审计夹具结构，再从真实互动选择器核对前 9 种出现且后 2 种不出现。其余受限类型只能
  由上文列出的原生或合法上下文子场景补齐，不得把不同上下文强行合并成“18+2”单场景。
