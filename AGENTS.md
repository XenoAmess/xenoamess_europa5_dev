...
- 因必要任务临时切换到在线模式后，该任务一完成，必须第一时间将 Steam 恢复为离线模式。
- Steam 处于在线模式时，启动任何游戏前必须检查当前账号是否已在其他机器上进行游戏。若账号正在游戏中，禁止启动游戏，也不得强制开始或挤占账号，以免顶号。
...
- 失败记录永久保留，不覆盖、不删除、不改写为 GREEN；记录至少包含 run ID、RED 分类、原因、替代结果和关键证据哈希。重试必须创建新 run。
- 完整运行证据仅保留至对应场景闭合。场景闭合前不得删除相关 run；闭合结论和上述失败记录写入受 Git 管理的报告后，应清理 `_runtime/` 中已被替代的完整运行目录。
